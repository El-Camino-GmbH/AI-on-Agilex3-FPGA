...
entity ONE_AI_Interface is
  generic (
    ...
    USE_COMPRESSED_INPUT : boolean := false                             -- Enable compressed input format (3 bytes/pixel)
  );
  port (
    clk              : in  std_logic;
    ...
    avm_burstcount : out std_logic_vector(7 downto 0) := (others => '0');
...
architecture rtl of ONE_AI_Interface is
  ...
  signal cycle_counter : natural range 0 to 52-5 := 0;
  constant PIXEL_DELAY_CYCLES : natural := 52-5;  -- 52 cycles in total per pixel
  ...
  -- FSM States
  type state_type is (idle_state, read_word_state, wait_readvalid_state, extract_pixel_state, pixel_delay_state);
  ...
  -- Avalon-MM Slave read data latch 
  signal avs_readdata_reg : std_logic_vector(31 downto 0) := (others => '0');

  -- Read guard must be a signal 
  signal avs_read_reg : std_logic := '0';
        ...
        case state is
          ...
          when read_word_state =>
            if avm_waitrequest = '0' then
              avm_read      <= '0'; -- stop request
              avm_read_i    <= '0';
              -- data is NOT valid here!
              --current_word  <= avm_readdata;
            end if;
            if avm_read_i = '0' then
              state <= wait_readvalid_state; -- go wait valid state
            end if;

          when wait_readvalid_state =>
          ...
          when extract_pixel_state =>
            ...
            if USE_COMPRESSED_INPUT then
              ...
              -- Uncompressed: 1 pixel per word (ignore upper byte)
              pixel_r_reg <= current_word(7 downto 0);
              pixel_g_reg <= current_word(15 downto 8);
              pixel_b_reg <= current_word(23 downto 16);
              cache_state <= "00";
            end if;
          ...
          when pixel_delay_state =>
            -- Wait 52 cycles, then output pixel data
            if cycle_counter >= PIXEL_DELAY_CYCLES then
              -- Transfer pixel data to output signals after 52 cycles (convert to 7-bit)
              ...
              if pixel_addr = LAST_PIXEL then
                ...
                if USE_COMPRESSED_INPUT and cache_state = "11" then
        ...
        if avs_read = '1' then
          ...
          case addr_int is
            when 0 =>
              ...
              avs_readdata_reg <= (31 downto 1 => '0') & new_image_ready;
              new_image_read <= '1';  -- Clear flag after reading
            when 1 =>
              -- Read: Outputs count
              avs_readdata_reg <= std_logic_vector(to_unsigned(OUTPUTS, 32));
            when 2 =>
              -- Read: FPS
              avs_readdata_reg <= std_logic_vector(to_unsigned(FPS, 32));
            when 3 =>
              if current_output = 0 then
                -- Read from output_data array
                avs_readdata_reg <= std_logic_vector(to_unsigned(output_data(current_user_position.Current_Height, current_user_position.Current_Width, current_user_position.Current_Objects, current_user_position.Current_Classes), 32));
              else
            ...
            when 16 to 31 =>
              ...
              if addr_int < OUTPUTS then
                avs_readdata_reg <= std_logic_vector(to_unsigned(current_user_position.Current_Width, 16)) & 
            ...
            when 32 to 47 =>
              ...
              if addr_int < OUTPUTS then
                avs_readdata_reg <= std_logic_vector(to_unsigned(current_user_position.Current_Objects, 16)) & 
            ...
            when 48 to 63 =>
              ...
              if addr_int < OUTPUTS then
                avs_readdata_reg <= std_logic_vector(to_unsigned(OUTPUT_DIMENSIONS(addr_int).Max_Height, 16)) & 
            ...
            when 64 to 79 =>
              ...
              if addr_int < OUTPUTS then
                avs_readdata_reg <= std_logic_vector(to_unsigned(OUTPUT_DIMENSIONS(addr_int).Max_Classes, 16)) & 
